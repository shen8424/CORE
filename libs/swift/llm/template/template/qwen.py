...
class Qwen2VLTemplate(Template):
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        encoded = super()._encode(inputs) # 只得到文本端的inputs_ids和label的ids
    ...
    def _post_encode(self, model, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.stage == 'stage2-pre' or self.stage == 'stage1':
            CL = []
            for short_expr, short_expr_negs in zip(inputs['short_expr'], inputs['short_expr_negs']):
                CL.append(short_expr)
                CL.extend(short_expr_negs)
            CL = self.processor.tokenizer(CL, padding=True, return_tensors="pt", add_special_tokens=False).to(input_ids.device)
            stage_inputs = {
                'input_ids': input_ids,
                'CL': CL,
            }
            for key in [
                'attention_mask', 'labels', 'position_ids', 'pixel_values', 'pixel_values_videos',
                'image_grid_thw', 'video_grid_thw', 'second_per_grid_ts'
            ]:
                if key in inputs and inputs[key] is not None:
                    stage_inputs[key] = inputs[key]
            return stage_inputs
        ...
        if self.stage == 'stage2-post':
            filtered = [sub for sub in inputs['conflict_val'] if sub]
            if not filtered:
                conflict_val_1, conflict_val_2 = None, None
            else:
                conflict_val_1, conflict_val_2 = map(list, zip(*filtered))

            if conflict_val_1 is not None:
                conflict_val_1 = self.processor.tokenizer(conflict_val_1, padding=True, return_tensors="pt", add_special_tokens=False).to(inputs_embeds.device)
                conflict_val_2 = self.processor.tokenizer(conflict_val_2, padding=True, return_tensors="pt", add_special_tokens=False).to(inputs_embeds.device)

            fake_image_box = []
            for box, shape in zip(inputs['fake_image_box'], inputs['image_shape']):
                if box and shape:  
                    x1, y1, x2, y2 = box
                    w, h = shape
                    normalized_coords = [x1 / w, y1 / h, x2 / w, y2 / h ]
                    fake_image_box.append(normalized_coords)

            mani_reg = self.processor.tokenizer(['manipulated region'], padding=False, return_tensors="pt", add_special_tokens=False).to(inputs_embeds.device)

            
            return {'inputs_embeds': inputs_embeds, 'conflict_val_1': conflict_val_1, 'conflict_val_2': conflict_val_2, 'image_grid_thw': image_grid_thw,
                    'fake_image_box': fake_image_box, 'conflict_src': inputs['conflict_src'], 'mani_reg': mani_reg, 'image_embeds': image_embeds, 'input_ids': input_ids}
