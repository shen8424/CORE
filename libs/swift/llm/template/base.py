...
class Template(ProcessorMixin):
    ...
    def encode(self,
        ...
        if self.mode in {'pt', 'train', 'prm', 'vllm', 'lmdeploy'}:
            encoded = self._encode_truncated(inputs) #参考这里
    ...
    def _encode_truncated(self, inputs):
        if self.mode in {'vllm', 'lmdeploy'}:
            encoded = Template._encode(self, inputs) #这里还没有被转化为inputs_ids
    ...
    def pre_forward_hook(self, model: nn.Module, args, kwargs):
        ...
        if 'inputs_embeds' in kwargs:
            # Save input_ids on model before removing, so forward can use it for image token masking
            saved_ids = kwargs.get('input_ids')
            model._saved_input_ids = saved_ids
            # Also save on base model (Qwen2_5_VLForConditionalGeneration) since that's where forward runs
            base_model = self.get_base_model(model)
            base_model._saved_input_ids = saved_ids
    ...
    def _data_collator_mm_data(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        pixel_values = [b['pixel_values'] for b in batch if b.get('pixel_values') is not None]
        if len(pixel_values) > 0:
            # 将所有 pixel_values 转换为 Tensor
            pixel_values = [torch.tensor(p) for p in pixel_values]
            res['pixel_values'] = torch.concat(pixel_values)

            image_sizes = [b['image_sizes'] for b in batch if b.get('image_sizes') is not None]
            if len(image_sizes) > 0:
                # 将 image_sizes 转换为 LongTensor
                image_sizes = [torch.tensor(s, dtype=torch.long) for s in image_sizes]
                res['image_sizes'] = torch.concat(image_sizes)

        pixel_values_videos = [b['pixel_values_videos'] for b in batch if b.get('pixel_values_videos') is not None]
        if len(pixel_values_videos) > 0:
            # 将 pixel_values_videos 转换为 Tensor
            pixel_values_videos = [torch.tensor(v) for v in pixel_values_videos]
            res['pixel_values_videos'] = torch.concat(pixel_values_videos)
        return res
    """
