...
def load_dataset(
    ...
    *,
    ...
    for dataset in datasets:
        dataset_syntax = DatasetSyntax.parse(dataset)  #处理完的dataset: 为路径
        use_hf = dataset_syntax.use_hf or use_hf_default
        # compat dataset_name
        if dataset_syntax.dataset in DATASET_MAPPING:   #没有做这步
            ...
            dataset_meta = dataset_syntax.get_dataset_meta(use_hf) # DatasetMeta(ms_dataset_id=None, hf_dataset_id=None, dataset_path=None, dataset_name=None, ms_revision=None, hf_revision=None, subsets=[SubsetDataset(name='default', subset='default', split=None, preprocess_func=None, is_weak_subset=False)], split=['train'], preprocess_func=<swift.llm.dataset.preprocessor.core.AutoPreprocessor object at 0x78c65132a770>, load_function=<function DatasetLoader.load at 0x78c65d2cb2e0>, tags=[], help=None, huge_dataset=False)
        load_function = dataset_meta.load_function 
        train_dataset = load_function(dataset_syntax, dataset_meta, **load_kwargs, use_hf=use_hf) # 这里起决定性因素
