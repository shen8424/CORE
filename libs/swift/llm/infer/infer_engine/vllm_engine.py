    ...
    def _prepare_engine_kwargs(
        ...
        **engine_kwargs,
        ...
        if 'device' in parameters:
            base_engine_kwargs['device'] = device
        base_engine_kwargs.update(engine_kwargs)
        unsupported_keys = [key for key in base_engine_kwargs if key not in parameters]
        for key in unsupported_keys:
            logger.warning(f'Ignoring unsupported vLLM EngineArgs parameter: {key}')
            base_engine_kwargs.pop(key)
        engine_args = engine_cls(**base_engine_kwargs)
